...
public class SM4 {
    private static String type = "GBK";
    ...
    private static String decryptSMS4toString(byte[] ciphertext, byte[] key) {
        ...
        try {
            return new String(plaintext, type);
    ...
    void SMS4Crypt(byte[] Input, byte[] Output, int[] rk) {
        ...
        for(j = 0; j < 32; j += 4) {
            int mid = x[1] ^ x[2] ^ x[3] ^ rk[j + 0];
            mid = this.byteSub(mid);
            x[0] ^= this.L1(mid);
            mid = x[2] ^ x[3] ^ x[0] ^ rk[j + 1];
            mid = this.byteSub(mid);
            x[1] ^= this.L1(mid);
            mid = x[3] ^ x[0] ^ x[1] ^ rk[j + 2];
            mid = this.byteSub(mid);
            x[2] ^= this.L1(mid);
            mid = x[0] ^ x[1] ^ x[2] ^ rk[j + 3];
            mid = this.byteSub(mid);
            x[3] ^= this.L1(mid);
        }
    ...
    private void SMS4KeyExt(byte[] Key, int[] rk, int CryptFlag) {
        ...
        for(r = 0; r < 4; ++r) {
            ...
            tmp[2] = Key[2 + 4 * r] & 255;
            tmp[3] = Key[3 + 4 * r] & 255;
            x[r] = tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3];
        }
        ...
        for(r = 0; r < 32; r += 4) {
            ...
            mid = this.byteSub(mid);
            int tmp206_205 = 0;
            int tmp215_214 = x[tmp206_205] ^ this.L2(mid);
            x[tmp206_205] = tmp215_214;
            rk[r + 0] = tmp215_214;
            mid = x[2] ^ x[3] ^ x[0] ^ this.CK[r + 1];
            mid = this.byteSub(mid);
            int tmp260_259 = 1;
            int tmp269_268 = x[tmp260_259] ^ this.L2(mid);
            x[tmp260_259] = tmp269_268;
            rk[r + 1] = tmp269_268;
            mid = x[3] ^ x[0] ^ x[1] ^ this.CK[r + 2];
            mid = this.byteSub(mid);
            int tmp314_313 = 2;
            int tmp323_322 = x[tmp314_313] ^ this.L2(mid);
            x[tmp314_313] = tmp323_322;
            rk[r + 2] = tmp323_322;
            mid = x[0] ^ x[1] ^ x[2] ^ this.CK[r + 3];
            mid = this.byteSub(mid);
            int tmp368_367 = 3;
            int tmp377_376 = x[tmp368_367] ^ this.L2(mid);
            x[tmp368_367] = tmp377_376;
            rk[r + 3] = tmp377_376;
        }
    ...
    private static byte[] encryptSMS4(String plaintext, byte[] key) {
        if (plaintext != null && !plaintext.equals("")) {
            ...
            try {
                length = plaintext.getBytes(type).length % 16;
            ...
            try {
                return encryptSMS4(sb.toString().getBytes(type), key);
